...
import {
   adminAddExperienceAction,
   adminDeleteExperienceAction,
...
function ExperienceRow({
   ...
   const updateAction = (formData: FormData) => {
      ...
      startUpdate(async () => {
         const result = await adminUpdateExperienceAction(init, formData);
         if (result?.error) {
   ...
   const deleteAction = (formData: FormData) => {
      ...
      startDelete(async () => {
         const result = await adminDeleteExperienceAction(init, formData);
         if (result?.error) {
   ...
   if (editing) {
      return (
         <form
            action={updateAction}
            style={{ padding: "10px", border: "1px solid var(--line)", borderRadius: 8, display: "grid", gap: 8 }}
         >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
               <input name="started_at" type="date" className="ws-form-input" defaultValue={dateInputValue(exp.started_at)} required />
               <input name="ended_at" type="date" className="ws-form-input" defaultValue={dateInputValue(exp.ended_at)} />
            </div>
      ...
      <div style={{ padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 8 }}>
         ...
         >
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)" }}>
               {yearRange(exp.started_at, exp.ended_at, exp.is_current)}
            </div>
...
export default function AddExperienceForm({
   ...
   const [state, action, pending] = useActionState(adminAddExperienceAction, init);
      ...
      <div style={{ display: "grid", gap: 14 }}>
         {editingId === null ? (
            <form action={action}>
               ...
               {state.success && (
                  <div style={{ color: "var(--teal)", fontSize: 13, marginBottom: 12 }}>เพิ่มประวัติการทำงานเรียบร้อย</div>
               )}
