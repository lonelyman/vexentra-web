...
import { adminAddSkillAction } from "@/app/actions/users";
...
export default function AddSkillForm({
   ...
   const [state, action, pending] = useActionState(adminAddSkillAction, init);
   ...
   return (
      <div style={{ display: "grid", gap: 14 }}>
         <form action={action}>
            ...
            {state.success && (
               <div style={{ color: "var(--teal)", fontSize: 13, marginBottom: 12 }}>เพิ่มทักษะเรียบร้อย</div>
            )}

            <div className="ws-form-group">
               <label className="ws-form-label">ชื่อทักษะ</label>
               <input name="name" className="ws-form-input" placeholder="เช่น Go, React, Docker" required />
            </div>
         ...
         <div style={{ borderTop: "1px solid var(--line)", paddingTop: 12 }}>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 8 }}>ทักษะที่มีอยู่ ({skills.length})</div>
            {skills.length === 0 ? (
